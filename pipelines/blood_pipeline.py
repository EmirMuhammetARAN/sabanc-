...
print("\n[3] LASSO (C=0.43739) ile ozellik secimi...")
lasso = LogisticRegression(penalty='l1', solver='liblinear', C=0.43739, random_state=42, max_iter=1000)
...
print(f"\n--- En Onemli 10 Gen (Gradient Boosting Agirliklari) ---")
importances = pd.Series(model.feature_importances_, index=selected_genes)
...
cv_pipeline = Pipeline([
    ('scaler', RobustScaler()),
    ('selector', SelectFromModel(LogisticRegression(penalty='l1', solver='liblinear', C=0.43739, max_iter=1000))),
    ('smote', SMOTE(k_neighbors=6, random_state=42)),
    ('model', GradientBoostingClassifier(learning_rate=0.1364, n_estimators=153, random_state=42))
])
...
plt.title('Kan Modulu - En Onemli 5 Gen (Optuna Optimized)')
